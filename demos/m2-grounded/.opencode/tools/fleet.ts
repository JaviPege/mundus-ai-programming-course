...
export const assign_mechanic = tool({
  ...
  async execute(args, context) {
    ...
    try {
      const ticket = db
        .prepare("SELECT bike_id FROM tickets WHERE id = ?")
        .get(args.ticket_id) as { bike_id: number } | undefined;
      if (!ticket) {
        return JSON.stringify({ ok: false, error: "ticket not found" });
      }
      db.prepare("UPDATE tickets SET mechanic = ?, status = 'assigned' WHERE id = ?").run(
        args.mechanic,
        args.ticket_id
      );
